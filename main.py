...
class BotLogic(BaseFunc):
    ...
    def send_start_message(self, chat_id):
        ...
        self.send_message(chat_id, "Добро пожаловать в автоматизированную справочную систему Университета "
                                   "Вернадского! Вы можете найти здесь основную информацию для "
                                   "абитуриентов/студентов, ссылки на различные электронные ресурсы и ЭИОС, "
                                   "а также задать любой интересующий Вас вопрос через систему обращений! Просто "
                                   "выбирайте нужный пункт через кнопки в нижней части экрана!\nСпасибо за Ваше "
                                   "участие в жизни Университета! Мы продолжаем работу и совершенствуем нашего бота!",
    ...
    def handle_custom_message_email(self, message, to_email):
        ...
        if 'waiting_for_contact_info' not in self.chat_states[chat_id]:
            ...
            self.send_message(chat_id, "ППожалуйста, в ОДНОМ сообщении, оставьте свои контактные данные: почту в "
                                       "формате xxxxxxx@xxxxxxx.xx (на неё Вам придёт ответ),  телефон в формате "
                                       "+7XXXXXXXXXX, и задайте Ваш вопрос."
                                       "\n\nВАЖНО: В рамках данного запроса Вы соглашаетесь на обработку ваших "
                                       "персональных данных службами  Университета, находящегося по адресу М.О. "
                                       "г.Балашиха, ул.Шоссе Энтузиастов, д.50, своих  контактных данных.",
                                       reply_markup=keyboard)  # И здесь
        else:
