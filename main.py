...
class BotLogic(BaseFunc):
    ...
    def send_start_message(self, chat_id):
        ...
        self.send_message(chat_id, f"Добро пожаловать в автоматизированную справочную систему Университета "
                                   f"Вернадского!\n\n Вы можете найти в нашем боте основную информацию для "
                                   f"абитуриентов/студентов, ссылки на электронные ресурсы и ЭИОС, а также задать "
                                   f"любой интересующий вопрос через систему обращений. Просто выбирайте нужный пункт "
                                   f"внизу экрана смартфона!\n\n Спасибо за Ваше участие в жизни Университета! Мы "
                                   f"продолжаем работу и совершенствуем нашего бота.",
    ...
    def handle_custom_message_email(self, message, to_email):
        ...
        if 'waiting_for_contact_info' not in self.chat_states[chat_id]:
            ...
            keyboard = self.create_keyboard(
                self.chat_states[chat_id]['current_state'] + [START_TEXT])
            self.send_message(chat_id, "Пожалуйста, в ОДНОМ сообщении, оставьте свои контактные данные: почту в "
                                       "формате xxxxxxx@xxxxxxx.xx (на неё Вам придёт ответ) и кратко опишите "
                                       "интересующий Вас вопрос вопрос."
